...
def traverse_directory(directory):
    ...
    for dir_path, dir_name, file_name in os.walk(directory):
        for f in file_name:
            ...
            results.append({'Path': f_path, 'Type': 'File', 'Size': size})
        for d in dir_name:
            d_path = os.path.join(dir_path, d)
            size = get_dir_size(d_path)
            results.append({'Path': d_path, 'Type': 'Directory', 'Size': size})
    return results
...
def save_results_to_json(results, file):
    with open (file, 'w') as f:
        json.dump(results, f)


def save_results_to_pickle(results, file):
    with open (file, 'wb') as f:
        pickle.dump(results, f)


def get_dir_size(new_path='.'):
    common_size = 0
    for dir_path, dir_name, file_name in os.walk(new_path):
        for file in file_name:
            f_path = os.path.join(dir_path, file)
            common_size += os.path.getsize(f_path)            
        for direct in dir_name:
            d_path = os.path.join(dir_path, direct)
            common_size += get_dir_size(d_path)            
    return common_size
